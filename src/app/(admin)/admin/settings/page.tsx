...
export default function AdminSettingsPage() {
  ...
  const accountForm = useForm<DepositAccountInput>({
  ...
  async function onSendAnnouncement(data: AnnouncementInput) {
    ...
    // Store announcement in platform_settings
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-[#1A1A2E]">Platform Settings</h1>
        <p className="text-sm text-[#666666] mt-1">Manage deposit account details and member announcements.</p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-xs text-[#666666]">
            Members see these details when making a deposit. Keep them up to date.
          </p>
        </CardHeader>
        <CardContent>
          {loadingAccount ? (
            ...
            <form onSubmit={accountForm.handleSubmit(onSaveAccount)} className="space-y-4 max-w-md">
              <div className="space-y-1">
                ...
                {accountForm.formState.errors.bank_name && (
                  <p className="text-xs text-[#E74C3C]">{accountForm.formState.errors.bank_name.message}</p>
                )}
              </div>
              <div className="space-y-1">
                ...
                <Input
                  ...
                  className={accountForm.formState.errors.account_number ? "border-[#E74C3C]" : ""}
                />
                {accountForm.formState.errors.account_number && (
                  <p className="text-xs text-[#E74C3C]">{accountForm.formState.errors.account_number.message}</p>
                )}
              </div>
              <div className="space-y-1">
                ...
                <Input
                  ...
                  className={accountForm.formState.errors.account_name ? "border-[#E74C3C]" : ""}
                />
                {accountForm.formState.errors.account_name && (
                  <p className="text-xs text-[#E74C3C]">{accountForm.formState.errors.account_name.message}</p>
                )}
              </div>
              <div className="space-y-1">
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-xs text-[#666666]">
            Send a notification to all members at once.
          </p>
        </CardHeader>
        <CardContent>
          <form onSubmit={announcementForm.handleSubmit(onSendAnnouncement)} className="space-y-4 max-w-md">
            <div className="space-y-1">
              ...
              <Input
                ...
                className={announcementForm.formState.errors.title ? "border-[#E74C3C]" : ""}
              />
              {announcementForm.formState.errors.title && (
                <p className="text-xs text-[#E74C3C]">{announcementForm.formState.errors.title.message}</p>
              )}
            </div>
            <div className="space-y-1">
              <Label>Message</Label>
              <textarea
                ...
                rows={4}
                className={`w-full border rounded-md px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#C2185B]/30 resize-none ${
                  announcementForm.formState.errors.body ? "border-[#E74C3C]" : "border-[#E0E0E0]"
                }`}
              />
              {announcementForm.formState.errors.body && (
                <p className="text-xs text-[#E74C3C]">{announcementForm.formState.errors.body.message}</p>
              )}
            </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base text-[#1A1A2E]">Announcement History</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {loadingAnnouncements ? (
            ...
            <div className="py-10 text-center text-[#666666] text-sm">
              <Clock size={32} className="text-[#666666]/30 mx-auto mb-2" />
