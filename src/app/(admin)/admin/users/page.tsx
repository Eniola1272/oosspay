...
interface UserWithBalance extends Profile {
  balance: number;
}
...
export default function AdminUsersPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-extrabold text-[#1A1A2E]">Manage Users</h1>
        <p className="text-sm text-[#666666] mt-1">{users.length} registered members</p>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[#E0E0E0]">
              {loading ? (
                ...
                filtered.map((u) => (
                  <tr key={u.id} className="hover:bg-[#FAFAFA] cursor-pointer" onClick={() => router.push(`/admin/users/${u.id}`)}>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-[#C2185B] text-white text-xs font-bold flex items-center justify-center shrink-0">
                    ...
                    <td className="px-4 py-3">
                      <Badge className={u.role === "admin" ? "bg-[#C2185B] text-white text-[10px]" : "bg-[#E0E0E0] text-[#666666] text-[10px]"}>
                        {u.role}
                      </Badge>
                    </td>
                    <td className="px-4 py-3">
                      <Button size="sm" variant="ghost" className="text-[#C2185B] hover:bg-[#FCE4EC]">
