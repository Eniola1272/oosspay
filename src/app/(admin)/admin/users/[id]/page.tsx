...
export default function AdminUserDetailPage() {
  ...
  async function loadData() {
    ...
    if (!profileRes.data) { router.push("/admin/users"); return; }
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-5 items-start">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-1.5">
                <Badge className={user.role === "admin" ? "bg-[#C2185B] text-white text-[10px]" : "bg-[#E0E0E0] text-[#666666] text-[10px]"}>
                  {user.role}
                </Badge>
              </div>
            </div>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base text-[#1A1A2E]">Record Deposit</CardTitle>
          </CardHeader>
          <CardContent>
                ...
                >
                  ...
                  {targets.filter((t) => t.status === "active").map((t) => (
                    <option key={t.id} value={t.id}>
                      {t.name} ({getSavingsProgress(t.current_amount, t.target_amount)}%)
                    </option>
                  ))}
                </select>
              </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base text-[#1A1A2E]">Savings Targets</CardTitle>
          </CardHeader>
          <CardContent>
            {targets.length === 0 ? (
                  ...
                  return (
                    <div key={t.id} className="space-y-1.5">
                      <div className="flex justify-between items-center gap-2">
                        ...
                        <Badge className={
                          t.status === "completed" ? "bg-[#27AE60]/10 text-[#27AE60] text-[10px] shrink-0" :
                          t.status === "cancelled" ? "bg-[#E0E0E0] text-[#666666] text-[10px] shrink-0" :
                          "bg-[#C2185B]/10 text-[#C2185B] text-[10px] shrink-0"
                        }>{t.status}</Badge>
                      </div>
                      ...
                      {t.deadline && (
                        <p className="text-xs text-[#666666]">Deadline: {formatDate(t.deadline)}</p>
                      )}
                    </div>
                  );
                })}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base text-[#1A1A2E]">Transaction History</CardTitle>
        </CardHeader>
                    ...
                    <tr key={tx.id} className="hover:bg-[#FAFAFA]">
                      ...
                      <td className="px-4 py-3">
                        <span className="flex items-center gap-1.5">
                          {tx.type === "deposit"
                            ? <ArrowDownLeft size={13} className="text-[#27AE60]" />
                            : <ArrowUpRight size={13} className="text-[#F39C12]" />}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base text-[#1A1A2E]">Withdrawal Requests</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
