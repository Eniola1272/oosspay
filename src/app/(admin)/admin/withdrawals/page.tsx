...
export default function AdminWithdrawalsPage() {
  ...
  async function handleAction(wr: WRWithUser, newStatus: WithdrawalStatus, note?: string) {
    ...
    // Notify user
    const notifTitle = newStatus === "approved" ? "Withdrawal Approved" :
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-extrabold text-[#1A1A2E]">Withdrawal Requests</h1>
        <p className="text-sm text-[#666666] mt-1">Review and process member withdrawal requests.</p>
      </div>
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          <div className="py-16 text-center text-[#666666] rounded-2xl border border-[#E0E0E0]">
          ...
          filtered.map((wr) => {
            ...
            return (
              <div key={wr.id} className="rounded-2xl border border-[#E0E0E0] overflow-hidden">
                {/* Card header */}
                <div
                ...
                {isExpanded && (
                  <div className="border-t border-[#E0E0E0] px-5 py-4 bg-[#FAFAFA] space-y-4">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        <p className="text-xs text-[#666666]">Bank</p>
                        <p className="font-medium text-[#1A1A2E]">{wr.bank_name}</p>
                      </div>
                      <div>
                        <p className="text-xs text-[#666666]">Account No.</p>
                        <p className="font-medium text-[#1A1A2E] tabular-nums">{wr.bank_account_number}</p>
                      </div>
                      <div>
                        <p className="text-xs text-[#666666]">Account Name</p>
                        <p className="font-medium text-[#1A1A2E]">{wr.bank_account_name}</p>
                      </div>
                      <div>
                        <p className="text-xs text-[#666666]">Reason</p>
                        <p className="font-medium text-[#1A1A2E]">{wr.reason || "—"}</p>
                      </div>
                    </div>
                    ...
                    {wr.status === "pending" && (
                      <div className="flex flex-wrap gap-2 pt-1">
                        <Button size="sm" className="bg-[#27AE60] hover:bg-[#219a52] text-white"
                          onClick={() => handleAction(wr, "approved")}>
                    ...
                    {wr.status === "approved" && (
                      <div className="flex gap-2 pt-1">
                        <Button size="sm" className="bg-[#C2185B] hover:bg-[#a31545] text-white"
                          onClick={() => handleAction(wr, "completed")}>
          ...
          <div className="space-y-3 mt-2">
            <div className="space-y-1">
              ...
              <textarea
                ...
                placeholder="Let the member know why this was declined..."
                rows={3}
            ...
            <div className="flex gap-3">
              <Button variant="outline" onClick={() => setReviewTarget(null)} className="flex-1" disabled={submitting}>
                Cancel
              </Button>
              <Button
                disabled={submitting}
