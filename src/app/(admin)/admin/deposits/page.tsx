...
export default function AdminDepositsPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-extrabold text-[#1A1A2E]">Deposit Requests</h1>
        <p className="text-sm text-[#666666] mt-1">
          Review receipt uploads and confirm member deposits.
        </p>
      </div>
        ...
        <TabsList className="bg-[#FAFAFA] border border-[#E0E0E0] flex-wrap h-auto gap-1 p-1">
          {TABS.map(({ label, value }) => {
            const count = value === "all"
              ? deposits.length
              : deposits.filter((d) => d.status === value).length;
            return (
              <TabsTrigger
                key={value}
                value={value}
        ...
        {loading ? (
          ...
          <div className="py-16 text-center text-[#666666] rounded-2xl border border-[#E0E0E0]">
            ...
            <p className="font-medium text-[#1A1A2E]">
              No {tab === "all" ? "" : tab} deposit requests
            </p>
          ...
          filtered.map((tx) => {
            ...
            const Icon = cfg.icon;
            return (
              <div key={tx.id} className="rounded-2xl border border-[#E0E0E0] overflow-hidden">
                {/* Card header */}
                <div
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#666666]">
                      {tx.profile?.email} · {formatDateTime(tx.created_at)}
                    </p>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <p className="font-extrabold text-lg text-[#1A1A2E] tabular-nums">
                    ...
                    <ChevronDown
                      size={16}
                      className={`text-[#666666] transition-transform ${isExpanded ? "rotate-180" : ""}`}
                    />
                  </div>
                </div>
                  ...
                  <div className="border-t border-[#E0E0E0] px-5 py-4 bg-[#FAFAFA] space-y-4">
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-semibold text-[#1A1A2E] tabular-nums">
                          {formatNaira(Number(tx.amount))}
                        </p>
                      </div>
                      <div>
                        ...
                        <p className="font-semibold text-[#1A1A2E]">
                          {tx.deposit_request_date ? formatDate(tx.deposit_request_date) : "—"}
                        </p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <a href={tx.receipt_url} target="_blank" rel="noopener noreferrer">
                          ...
                          <img
                            src={tx.receipt_url}
                            alt="Deposit receipt"
                            className="max-h-52 rounded-xl border border-[#E0E0E0] object-contain hover:opacity-90 transition-opacity cursor-pointer"
                          />
                        </a>
                      </div>
                    ) : (
          ...
          <div className="space-y-3 mt-2">
            <div className="space-y-1">
              ...
              <textarea
                ...
                placeholder="e.g. Receipt doesn't match claimed amount..."
                rows={3}
