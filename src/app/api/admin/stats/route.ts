...
export async function GET() {
  ...
  // Calculate total savings from all completed transactions
  ...
  const txActivity = (recentTxRes.data ?? []).map((t: any) => ({
    id: `tx-${t.id}`, type: t.type === "deposit" ? "deposit" : "withdrawal_request",
    description: `${t.type === "deposit" ? "Deposit" : "Withdrawal"} for ${t.profiles?.full_name ?? "unknown"}`,
    amount: Number(t.amount), created_at: t.created_at,
  }));
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const wActivity = (recentWRes.data ?? []).map((w: any) => ({
    id: `wr-${w.id}`, type: "withdrawal_request",
    description: `Withdrawal request from ${w.profiles?.full_name ?? "unknown"}`,
    amount: Number(w.amount), created_at: w.created_at,
  }));
  ...
  return NextResponse.json({ stats: { totalUsers, totalSavings, pendingWithdrawals, todayDeposits }, activity });
}
