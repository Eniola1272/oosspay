...
interface Stats { totalUsers: number; totalSavings: number; pendingWithdrawals: number; todayDeposits: number }
interface ActivityItem { id: string; type: "deposit" | "withdrawal_request" | "new_user"; description: string; amount?: number; created_at: string }
...
export default function SuperAdminDashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (res.ok) { const data = await res.json(); setStats(data.stats); setActivity(data.activity); }
      setLoading(false);
    }
    load();
  }, []);
  ...
  const statCards = [
    ...
    { label: "Today's Deposits",    value: stats?.todayDeposits ?? 0,      icon: TrendingUp, color: "text-amber-600 bg-amber-50",     fmt: (v: number) => formatNaira(v) },
  ];

  return (
    <div>
      <AdminTopBar title="Super Admin Overview" subtitle="Full platform visibility and audit access." />
          ...
          {statCards.map(({ label, value, icon: Icon, color, fmt }) => (
            <Card key={label} className="border-[#E0E0E0]">
              <CardContent className="p-5 space-y-3">
                ...
                {loading ? <Skeleton className="h-7 w-24" /> : <p className="text-2xl font-extrabold text-[#1A1A2E] tabular-nums">{fmt(value)}</p>}
                ...
                {activity.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="px-6 py-4 flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${iconColor}`}><Icon size={14} /></div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-[#1A1A2E]">{item.description}</p>
                        <p className="text-xs text-[#666666]">{formatDateTime(item.created_at)}</p>
                      </div>
                      {item.amount != null && <span className="text-sm font-bold tabular-nums text-[#1A1A2E] shrink-0">{formatNaira(item.amount)}</span>}
                    </div>
                  );
                })}
              </div>
