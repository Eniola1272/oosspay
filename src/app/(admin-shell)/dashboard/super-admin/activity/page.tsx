...
const ACTION_CONFIG: Record<ActionType, { label: string; verb: string; icon: typeof Clock; iconColor: string; badgeColor: string }> = {
...
export default function ActivityLogPage() {
  ...
  async function load() {
    ...
    const [txRes, wrRes] = await Promise.all([
      sb.from("transactions").select("id, amount, status, admin_note, created_at, user_id, recorded_by")
        .eq("type", "deposit").not("recorded_by", "is", null).order("created_at", { ascending: false }).limit(100),
      sb.from("withdrawal_requests").select("id, amount, status, admin_note, reviewed_at, user_id, reviewed_by")
        .not("reviewed_by", "is", null).order("reviewed_at", { ascending: false }).limit(100),
    ]);

    const txRows: { id: string; amount: number; status: string; admin_note: string | null; created_at: string; user_id: string; recorded_by: string }[] = txRes.data ?? [];
    const wrRows: { id: string; amount: number; status: string; admin_note: string | null; reviewed_at: string; user_id: string; reviewed_by: string }[] = wrRes.data ?? [];

    const adminIds = new Set<string>([...txRows.map((r) => r.recorded_by), ...wrRows.map((r) => r.reviewed_by)]);
    const memberIds = new Set<string>([...txRows.map((r) => r.user_id), ...wrRows.map((r) => r.user_id)]);
    const allIds = [...new Set([...adminIds, ...memberIds])];

    let profiles: { id: string; full_name: string; email: string }[] = [];
    if (allIds.length > 0) {
      const { data } = await sb.from("profiles").select("id, full_name, email").in("id", allIds);
      profiles = data ?? [];
    }
    ...
    const depositEntries: LogEntry[] = txRows.map((r) => ({
      id: `tx-${r.id}`, actionType: depositAction(r.status), amount: Number(r.amount),
      adminId: r.recorded_by, adminName: byId(r.recorded_by).full_name,
      memberId: r.user_id, memberName: byId(r.user_id).full_name, memberEmail: byId(r.user_id).email,
      note: r.admin_note, timestamp: r.created_at,
    }));

    const withdrawalEntries: LogEntry[] = wrRows.map((r) => ({
      id: `wr-${r.id}`, actionType: withdrawalAction(r.status), amount: Number(r.amount),
      adminId: r.reviewed_by, adminName: byId(r.reviewed_by).full_name,
      memberId: r.user_id, memberName: byId(r.user_id).full_name, memberEmail: byId(r.user_id).email,
      note: r.admin_note, timestamp: r.reviewed_at,
    }));

    setEntries([...depositEntries, ...withdrawalEntries].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()));
    setLoading(false);
  }
    ...
    <div>
      ...
      <div className="p-4 lg:p-6 space-y-4 lg:space-y-5">
        <div className="flex items-center gap-2 text-xs text-[#666666] bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 w-fit">
          <Filter size={13} className="text-amber-500" /> Super Admin view only
        </div>
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as TabValue)}>
          <TabsList className="bg-[#FAFAFA] border border-[#E0E0E0] h-auto gap-1 p-1">
            {([
              ...
              const count = value === "all" ? entries.length : entries.filter((e) => e.actionType.startsWith(value.slice(0, -1))).length;
              return (
                <TabsTrigger key={value} value={value}
                  className="text-xs data-[state=active]:bg-amber-500 data-[state=active]:text-white gap-1.5">
                  {label}
                  {count > 0 && <span className="bg-current/20 rounded-full px-1.5 py-0.5 text-[10px] font-bold leading-none">{count}</span>}
                </TabsTrigger>
              );
            })}
          </TabsList>
        ...
        <div className="rounded-2xl border border-[#E0E0E0] overflow-hidden bg-white">
          {loading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-sm text-[#666666] mt-1">Actions will appear here once admins start reviewing deposits and withdrawals.</p>
            </div>
          ) : (
            <div className="divide-y divide-[#F0F0F0]">
              {filtered.map((entry) => {
                ...
                return (
                  <div key={entry.id} className="px-5 py-4 flex items-start gap-4 hover:bg-[#FAFAFA] transition-colors">
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${cfg.iconColor}`}>
                      {isTx ? <ArrowDownLeft size={15} /> : <ArrowUpRight size={15} />}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      {entry.note && (
                        <p className="text-xs text-[#666666] mt-1 bg-[#F5F5F5] rounded px-2 py-1 inline-block">Note: {entry.note}</p>
                      )}
